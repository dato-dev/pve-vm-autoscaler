...
| L26 | ~~Все зависимости стоят как `"latest"`~~ — диапазоны зафиксированы в M1.10. **`package-lock.json` всё ещё отсутствует.** | Патч- и минорные версии резолвятся заново на каждом прогоне; точной воспроизводимости нет, `npm ci` и кэш недоступны. | M1.10a |
...
- [ ] **M1.10a** ⚠️ **Действие вне репозитория:** сгенерировать и закоммитить
      `package-lock.json` — нужен `npm`, которого нет в окружении агента:

      ```bash
      npm install && git add package-lock.json && git commit -m "build(deps): зафиксировать версии в lockfile"
      ```

      После этого CI сам переключится на `npm ci`, и останется включить `cache: npm`
      в обоих `setup-node` (одна строка).
...
### Milestone 6 — Наблюдаемость и API
