...
| L12 | **Нет CI, нет git-репозитория**, нет версионирования и CHANGELOG. | Нечего проверять на PR; версия проставляется вручную. | M1.7 |
...
- [ ] **M1.7** Git-репозиторий + CI (build / test / lint на push). *(L12)*
      Вместе с ним — **semantic-release**: Conventional Commits, автоверсия и CHANGELOG из истории,
      `commitlint` + husky на `commit-msg`, проверка формата в CI для PR.
      Конвенция описана в [CLAUDE.md](CLAUDE.md), п. 1.5.
