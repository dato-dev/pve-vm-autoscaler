...
| ~~L10~~ | ~~**Диск при resize захардкожен как `virtio0`.**~~ Закрыто в M2.10. | — | ✅ |
...
| ~~L22~~ | ~~**`sampleIntervalSeconds` объявлен в схеме и лежит в примере, но не читается кодом.** Проверено: единственные вхождения — `packages/shared/src/index.ts:36` и сам пример.~~ Закрыто в M2. | — | ✅ |
| ~~L23~~ | ~~**Policy в JSON: комментарии невозможны.** В одном объекте смешаны тюнинг (пороги, окна) и шаблон машины (`templateVmId`, cores, memory, storage); шаблон нельзя переиспользовать между политиками.~~ Закрыто в M2. | — | ✅ |
| ~~L24~~ | ~~**Термин «node» означает две разные сущности.** `proxmox.targetNode` — гипервизор, `minNodes`/`maxNodes` — воркер-VM.~~ Закрыто в M2. | — | ✅ |
| ~~L25~~ | ~~**Пример политики содержит отладочные значения**: `evaluationWindowSeconds: 5`, `cooldownSeconds: 10`.~~ Закрыто в M2. | — | ✅ |
...
- [x] **M2.1** Загрузчик читает YAML парсером `yaml` (стандарт 1.2). *(L23)*
- [x] **M2.2** `nodeTemplates` отделены от `policies` и связаны по имени. *(L23)*
- [x] **M2.3** Поле `version: 1` в корне.
- [x] **M2.4** Duration-строки: `window: 2m`, `cooldown: 1h30m`.
- [x] **M2.5** Quantity-строки: `memory: 2Gi`, `disk: 20Gi`.
- [x] **M2.6** Проценты со знаком `%`, суффикс обязателен.
- [x] **M2.7** Мёртвое поле `sampleIntervalSeconds` не перенесено. *(L22)*
- [x] **M2.8** `hypervisor` вместо `targetNode`, `nodes.min`/`nodes.max`, плоский `selector`. *(L24)*
- [x] **M2.9** Пороги в `scaleUp`; секция `scaleDown` заведена заранее под Milestone 4.
- [x] **M2.10** `diskDevice` в шаблоне машины вместо захардкоженного `virtio0`. *(L10)*
- [x] **M2.11** Ошибки собираются списком с путями до полей и текстом парсера единиц.
- [x] **M2.12** `npm run policy:validate` — проверка без запуска сервера и базы.
- [x] **M2.13** `npm run policy:convert` — конвертер старого формата, валидирует результат перед выводом.
- [x] **M2.14** Новый `infra/policy.example.yaml` с комментариями; отладочные 5s/10s убраны. *(L25)*
- [x] **M2.15** 97 тестов: парсеры единиц, схема, разрешение шаблонов, загрузчик, конвертер, решение.
